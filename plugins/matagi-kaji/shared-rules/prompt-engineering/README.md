...
（AGENTS.md/CLAUDE.md/rules/skills/subagents 等の選択）は `shared-rules/harness-engineering/selection-rule.md` を参照する。
...
各ルールは `[[composition-rule]]` のように `[[wikilink]]` で相互参照される（記法の定義は [[directory-rule]]（`matagi/shared-rules/rules-directory/directory-rule.md`）「相互リンク記法」）。
