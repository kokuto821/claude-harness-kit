...
- **合意を得るまで実装に入らない。** issue の範囲を超える変更が必要だと判明したら、ここで別 issue 化を提案する（[[issue-driven-rule]] 粒度基準）。
...
- **完了フェーズ**: 現フェーズ、作成したブランチ名、委譲先スキルと結果、レビュー指摘の適用状況（省略した場合はその旨）、PR の URL、マージ結果、issue のクローズ状態、完了条件の充足状況（未達があれば残課題）
