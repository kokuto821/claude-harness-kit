...
3. 共通必読を読む。まず思想・分類の `UI哲学.md` と `architecture/ui-architecture.md`、続いて `styling-rule/` の `color.md` / `space-and-radius.md` / `button-rule.md` / `improve-layout.md` / `improve-ui.md`。これらはどのコンポーネントでも適用するため毎回読む。
4. 索引から、対象 UI に該当する `component/` ルールを読む。
5. 実装をルールと照合し、違反・逸脱を洗い出す。共通必読を除き、読み込むのは該当ルールだけに絞り、全ファイルは読み込まない。

## チェック観点

- **思想・原則**: `UI哲学.md` の方針（何を美しい・正しい UI とするか）に照らして、設計判断が思想と矛盾していないか。
