...
**開発は「issue 作成フェーズ」と「issue 実行フェーズ」に分離し、変更は issue を起点に始める。実行フェーズの作業は、必ず対象 issue に紐づくブランチ上で行う。** 作成フェーズはやりたいことを issue という成果物に落として終わり、実行フェーズは既存の issue を1件選んでブランチ・PR・クローズまで閉じる。**同じやり取りの中で作成フェーズから実行フェーズへ続けて進まない**（issue を作った勢いで実装に入らない）。
...
| 実行フェーズ中に、対象 issue の作業のついでに直せる軽微な修正（typo 1箇所など） | 別 issue 化せず、そのブランチの変更に含めてよい |
| 軽微な修正だが、単独でコミットすることになる | issue 化する。issue の無い変更を既定ブランチに積まない |
...
| 実行フェーズ中に、対象 issue のスコープ外の課題が見つかった（軽微ではない） | その PR に混ぜない。**issue 化を検討し、判断材料とあわせてユーザーに提案する**（上表の基準を再適用する）。合意を得るまで着手しない |
...
**既定ブランチ（`main` / `master` など）で直接作業しない。作業に取り掛かる前に、対象 issue に紐づくブランチへ移動する。** 移動は実装方針の検討より前に行う。既定ブランチ上のままでよいのは、対象 issue の選択と内容確認までとする。

| 状況 | どうする |
|------|---------|
| 既定ブランチ上にいる | 作業に着手する前にブランチを切る。すでに変更してしまっていたら、退避（stash / commit）してからブランチを切り、そこへ移す |
| すでにブランチ上にいるが、対象 issue に紐づかないブランチ | そのまま続けない。対象 issue のブランチを切り直してから進める |
| 紐づく issue が無いまま変更を始めようとしている | 作成フェーズに戻り、まず issue を立てる（「issue 化すべきか」の判断基準に従う） |
...
- **既定ブランチ（`main` / `master` など）で直接ファイルを変更する・コミットする**
...
「必ず issue に紐づくブランチで作業する」節は、issue #12 実装中のユーザー指示で追加した。あわせて、軽微な修正を issue 化しない例外が既定ブランチ直接作業の抜け穴にならないよう、単独コミットになる場合は issue 化する側へ寄せた。
