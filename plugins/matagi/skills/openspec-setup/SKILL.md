...
description: >
  ...
  openspec init・schemaのローカルfork・matagiのルールファイルへの
...
作業ディレクトリに OpenSpec（`@fission-ai/openspec`）を導入し、`openspec-workflow`（`skills/openspec-workflow/SKILL.md`）で使う前提を整える。matagi がマーケットプレイスプラグインとして導入されているどの作業リポジトリでも、同じ手順で同じ内容の `openspec/config.yaml`・ローカル fork したスキーマを再現できるようにする。
...
2. **既存導入の確認（冪等性）** — 対象ディレクトリに `openspec/` が既にあれば、以下で不足分を特定し、以降の手順はその不足分のみ実施する（フルスクラッチではないことをユーザーに伝える）。
   ...
   - `openspec/schemas/spec-driven/templates/proposal.md` に `matagi:` を含む HTML コメントがあるか（`grep -l "matagi:" openspec/schemas/spec-driven/templates/*.md`） → 無ければ手順6
...
6. **テンプレートへのポインタ埋め込み** — フォークしたテンプレート（`openspec/schemas/spec-driven/templates/{proposal,design,tasks}.md`）に、`<!-- matagi: ... -->` 形式の HTML コメントで参照ポインタを埋め込む(ルール本文はコピーしない)。**埋め込む内容は [[openspec-rule]] §導入 の表を唯一の正とし、本手順には再掲しない**（対象 issue 番号の明記が不要な作業ディレクトリでは、`proposal.md` 側のみユーザーに確認のうえ省略してよい）。
