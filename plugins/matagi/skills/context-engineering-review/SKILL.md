...
description: >
  ...
  と言われたとき、スキル・サブエージェント定義・AGENTS.md/CLAUDE.md・プロンプトを
...
プロンプト本文の品質そのものは `prompt-review`、ステアリング手法の選択は `harness-review` の担当。
本スキルは「有限なコンテキストをどう管理・キュレーションするか」のレンズに限る。
3観点（プロンプト品質・ステアリング手法・コンテキスト設計）をまとめて見る場合は `ai-engineering-review`。
...
対象のスキル／サブエージェント定義／AGENTS.md/CLAUDE.md／プロンプトを特定する。差分が分かる場合は変更箇所を優先する。
