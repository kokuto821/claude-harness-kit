...
- 各原則の定義を本ルールや参照元（実装・テスト・レビューのルールやエージェント）に再掲する。定義は [[design-principles-glossary]] を唯一の正とし、本ルールは観点のみを持つ（値の二重管理を避ける。[[externalization-rule]]（`matagi-kaji/shared-rules/rule-externalization/externalization-rule.md`）§単一情報源）。
